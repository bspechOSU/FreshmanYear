...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        if (!(n.compareTo(m) == 0) || !m.isZero()) {
            // This ensures that m isn't 0 and that they aren't equal
            NaturalNumber newN = n.divide(m);
            // starts the formula necessary
            if (!newN.isZero()) {
                n.copyFrom(newN);
                reduceToGCD(m, n);
                // recursive formula
            ...
            n.copyFrom(m);
            // ensures n will eventually get the proper value needed
        }
        m.clear();
        //clears m
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.compareTo(new NaturalNumber2(0)) == 0) {
            ...
            NaturalNumber modP = n.newInstance();
            modP = p.divide(new NaturalNumber2(2));
            // grabs the remainder
            NaturalNumber copyOfN = n.newInstance();

            if (modP.compareTo(new NaturalNumber2(1)) == 0) {
                copyOfN.copyFrom(n);
            }
            //returns n^(p/2)
            powerMod(n, p, m);
            ...
            //squares n^(p/2)
            NaturalNumber tempN = n.newInstance();
            tempN.copyFrom(n);
            n.multiply(tempN);

            //if odd power: multiply by original n
            if (modP.compareTo(new NaturalNumber2(1)) == 0) {
                n.multiply(copyOfN);
            }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean ret = false;
        NaturalNumber nTwo = new NaturalNumber2(n);
        // duplicating variables because two routes can be taken
        NaturalNumber wTwo = new NaturalNumber2(w);

        NaturalNumber wThree = new NaturalNumber2(w);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber one = new NaturalNumber2(1);
        powerMod(wTwo, two, n);
        // checks the first route
        if (wTwo.compareTo(one) == 0) {
            ret = true;
        }
        nTwo.decrement();
        powerMod(wThree, nTwo, n);
        // checks the second route
        if (wThree.compareTo(one) != 0) {
            ret = true;
        }
        /*
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 1; i < fifty; i++) {
            // loop enters and continues if it could still be a prime
            if (prime2) {
                NaturalNumber randomN = randomNumber(n);
                // makes a random number
                if ((randomN.compareTo(one) > 0)
                        && randomN.compareTo(nDec) < 0) {
                    // checks to make sure the random number fits the precondition
                    prime2 = !isWitnessToCompositeness(randomN, n);
                }
            }
        }
        return prime2;
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        boolean prime = false;
        // the loop will continue until prime becomes true
        while (!prime) {
            boolean prime1 = isPrime1(n);
            boolean prime2 = isPrime2(n);
            if (prime1 && prime2) {
                prime = true;
                // if both prime checks return true, the value could be a prime
            } else {
                n.increment();
                n.increment();
                // adds 2
