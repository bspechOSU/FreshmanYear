...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String ret = "";
        // starts empty
        ret = str1.substring(0, str1.length() - overlap);
        // gets the first part not including the overlap
        ret = ret + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //Goal: Run through the current set looking for sub strings
        Set<String> holdingSet = strSet.newInstance();
        // new set
        boolean sub = false;
        // sub variable
        int tracker = 0;
        while (strSet.size() > 0) {
            // loop continues until strSet is empty
            String checker = strSet.removeAny();
            if ((checker.indexOf(str) < 0 && str.indexOf(checker) < 0)) {
                // if there is no overlap, the check gets passed into the new set
                holdingSet.add(checker);
            } else {
                // if there is overlap this occurs
                sub = true;
                if (checker.indexOf(str) >= 0) {
                    // if the check contains str, check gets added
                    holdingSet.add(checker);
                    if (holdingSet.contains(str)) {
                        // if str was already added in a different run through
                        // it will get removed to ensure so substrings exist
                        String dead = holdingSet.remove(str);
                    }
                } else if (tracker == 0) {
                    // we only want to add str once if its the appropriate sub
                    holdingSet.add(str);
                    tracker = 1;
                }
            }
        ...
        if (!sub) {
            // if its never a substring it gets added outright
            holdingSet.add(str);
        }

        strSet.transferFrom(holdingSet);
        // transfer everything back to the original set
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> combination = new Set1L<>();
        combination.add(input.nextLine());
        while (!input.atEOS()) {
            // input will be collected until its at the end of stream
            String check = input.nextLine();
            // gets the next line
            addToSetAvoidingSubstrings(combination, check);
            // checks if its valid to be added or not

        }

        return combination;
        // returns the combination of inputs in a set
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            // checks every character to see of its a ~ or not
            char checkChar = text.charAt(i);
            if (checkChar == '~') {
                // if its a squiggle it will print a new line
                out.println();
            } else {
                out.print(checkChar);
            }
